...
- **CardEditDialog.tsx** — Per-card field override dialog opened from PreviewStep; accepts `overrides` and `fontSizeOverrides` keyed by binding. Also requires `template: Template` and `printSettings` props to render a live `CardCanvas` preview. The preview spans the **full dialog width** at the top (scaled to 880px container width), with the form fields below. Uses the same scale/clip pattern as `PreviewGrid` (render canvas at full mm→px size, then CSS `transform: scale(transformOrigin: 'top left')`). `previewRecord` is built via `useMemo` from the current edit state so the preview updates as the user types.
